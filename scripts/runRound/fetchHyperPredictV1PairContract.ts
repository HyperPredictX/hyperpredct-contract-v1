import { ethers, artifacts, network } from "hardhat";
import { loadConfig } from "../lib/loadConfig";

type CallerType = "operator" | "admin";

export async function fetchHyperPredictV1PairContract(
  callerType: CallerType = "operator"
) {
  const config = await loadConfig();
  const networkName = network.name;
  const contractAddress = config.HyperPredictV1Pair[networkName];
  const [admin, operator] = await ethers.getSigners();
  console.log(`Predict Contract: ${contractAddress}`);
  console.log(`admin   : ${admin.address}`);
  console.log(`operator: ${operator.address}`);

  // Attach ABI
  const artifact = await artifacts.readArtifact("HyperPredictV1Pair");
  const caller = callerType === "admin" ? admin : operator;
  const HyperPredictV1PairContract = new ethers.Contract(
    contractAddress,
    artifact.abi,
    caller
  );

  return { HyperPredictV1PairContract };
}
